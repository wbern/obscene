...
describe("formatIgnoreFile", () => {
  ...
  it("formats empty patterns with placeholder message", () => {
    const result = formatIgnoreFile([]);

    expect(result).toContain("# Generated by obscene init");
    expect(result).toContain("# No project-specific patterns detected.");
    expect(result).toContain("# Add glob patterns here, one per line.");
  });
