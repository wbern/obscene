...
describe("formatHotspotsTable correlations section", () => {
  it("renders a correlations table with ρ, N, and confidence stamps", () => {
    const output: HotspotsOutput = {
      generated: "2026-01-01T00:00:00.000Z",
      guide: {
        correlations:
          "Spearman rank correlation between each metric × churn ranking and Fix Activity × Churn.",
      },
      churnWindow: "3 months",
      rankings: {
        complexity: {
      ...
      correlations: {
        reference: "defects",
        referenceLabel: "Fix Activity × Churn",
        entries: [
          {
            metric: "complexity",
            label: "Complexity × Churn",
            rho: 0.7234,
            n: 12,
            confidence: { ...STUB_CONFIDENCE, level: "weak" },
          },
          {
            metric: "nesting",
            label: "Nesting × Churn",
          ...
          {
            // |ρ| ≤ 0.5 — renders without the red/green coloring.
            metric: "authors",
            label: "Authors × Churn",
            rho: 0.1234,
            n: 12,
    ...
    expect(result).toContain("FIX ACTIVITY × 🔄 CHURN");
    expect(result).toContain("Spearman rank correlation");
    expect(result).toContain("Complexity × Churn");
    expect(result).toContain("Nesting × Churn");
    expect(result).toContain("0.723");
    expect(result).toContain("-0.651");
    expect(result).toContain("WEAK");
    // High-positive ρ is colored red (signal warning), strong-negative green.
    // Both colored values pass through pc; assert raw values exist either way.
  });

  it("renders an empty-state message when no other rankings are available", () => {
    const output: HotspotsOutput = {
      generated: "2026-01-01T00:00:00.000Z",
      guide: {},
      churnWindow: "3 months",
    ...
    const result = formatHotspotsTable(output);
    expect(result).toContain("No other rankings available to correlate.");
  });

  it("renders a skipped notice when correlations are surfaced in skipped[]", () => {
    const output: HotspotsOutput = {
      generated: "2026-01-01T00:00:00.000Z",
      guide: {},
      churnWindow: "3 months",
      ...
      skipped: {
        correlations: {
          reason:
            "Fix Activity × Churn unavailable — no reference ranking to correlate against",
          confidence: STUB_INCONCLUSIVE,
        },
      },
    };
    const result = formatHotspotsTable(output);
    expect(result).toMatch(/Correlations\s+—\s+skipped/);
    expect(result).toContain("no reference ranking to correlate against");
    // No "Correlations × Churn" — it's not a ranking and shouldn't be labelled
    // as one.
    expect(result).not.toContain("Correlations × Churn");
    expect(result).not.toContain("Correlations × Churn");
  });
});
