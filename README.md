...
>
> ```
> infra/files/admin-api/app.py     Score=0.3561  Δ=-49  (refactor-net-down)
> frontend/src/App.jsx             Score=0.3409  Δ=+76  (added persona icons + drawer)
> infra/files/admin-api/provisioner.py  Score=0.2616  Δ=+26
...
>
> **`--full-delta` — worked, no fallback, surfaces movement that score-only sorting hides.** Structured tier transitions came back populated: 7 files entered HOT, 5 exited HOT, 22 new files, 0 deleted, corpus-complexity Δ = +193. The interesting finding was *which* files entered HOT — three were test files: `test_ttyd_ports.py` (+152%), `test_claude_launch.py` (+107%), `test_screen_mcp.py` (+107%). Reading the commit log confirms these were stabilization runs ("test: pin X", "test: widen Y"), not regressions. The relative-percentile caveat in the README (`scoreChanges` carries absolute deltas precisely because relative tier movement can be misleading) is doing real work here: the test files' *absolute* score changes were small; their *tier* movement was large because the rest of the corpus moved less. Reading both columns prevents an obvious misread ("our tests are getting hot" → no, the corpus just stabilized around them).
>
...
>
> **Fix Activity caveat refinement: even at ~99% conventional-commits adoption, there's still a gap.** Tally across 129 commits: 64 feat, 36 fix, 11 test, 7 refactor, 5 docs, 3 chore, 1 perf, 1 style, 1 revert = ~99% prefixed. By the prior report's logic, Fix Activity should be near-canonical signal. Spot-checking the top-ranked file (admin-api/app.py, 7 `fix:` commits in window): scanning all 40 commits touching that file turns up 2–3 `refactor:` commits whose messages absorb bug fixes ("unify tail window + fold offer into focus", "polish restart-button feature") — fixes bundled into a refactor narrative don't get the `fix:` prefix. So even at 99% adoption, Fix Activity probably undercounts real fixes by 20–30% on this file. Not a defect in the metric; a documentable limit that doesn't go away just because convco discipline is high.
>
> **Coupling rediscovered the documented operational hazard.** Acknowledging the first field report already covered this pattern at a different scale — worth confirming it reproduces. Top pair: `provisioner.py` ↔ `provision-student.yml` at 11 shared commits, **91.7% degree**. The repo's own CLAUDE.md documents this exact pair as "Pitfall #10": *"Both paths must produce identical per-student state. Drifted twice."* Obscene found it without being told. Three of the next four HOT pairs all involve the same triangle (`provisioner.py` + `provision-student.yml` + `append.md.j2` + `student-claude.json.j2`) — the entire student-provisioning surface area, surfaced as a co-change cluster the developer should keep an eye on. The pattern is real and reproducible across codebase instances.
>
> **scc per-language sanity check.** Reading `obscene report` per-language: Python and JSX render normally. Jinja templates show up with non-zero complexity (e.g. `append.md.j2` at complexity 14) — scc *is* counting `{% if %}` / `{% for %}`, which is correct. Terraform shows up declarative-low (density ~0.20, complexity ≈ nesting). Ansible YAML doesn't appear in the top 50 because scc gives it modest complexity numbers per file. None of this looked implausible. The v2.4.0 zero-complexity-nesting filter held — no jinja-with-zero-control-flow files leaked into HOT this time either.
>
